	...
	drwrap_init();
	{
        ...
        while (s != NULL) {
            if (dr_sscanf(s, PIFX, (ptr_uint_t *)&addr) == 1 ||
                ...
                dr_fprintf(f_global, "replacing %s @"PFX" in executable from options\n",
                ...
                app_pc addr, int index _IF_DEBUG(bool sym_processed_once))
{
    ...
    if (addr >= mod->end) {
        dr_fprintf(f_global, "NOT replacing %s @"PFX" beyond end of mapping for module %s\n",
            replace_routine_name[index], addr, modname == NULL ? "<noname>" : modname);
        return;
    }
    dr_fprintf(f_global, "%s %s @"PFX" in %s (base "PFX")\n",
    ...
    do {
        ...
        if (last_was_call) {
            ...
            if (instr_get_opcode(&inst) == OP_add &&
                ...
                opnd_get_reg(instr_get_dst(&inst, 0)) == DR_REG_XBX) {
                addr_got = opnd_get_immed_int(instr_get_src(&inst, 0)) + prev_pc;
                dr_fprintf(f_global, "\tfound GOT "PFX" for indir func %s\n",
        ...
        if (addr_got != NULL &&
            ...
            opnd_get_base(instr_get_src(&inst, 0)) == DR_REG_XBX &&
            opnd_get_index(instr_get_src(&inst, 0)) == DR_REG_NULL &&
            opnd_get_scale(instr_get_src(&inst, 0)) == 0) {
            app_pc addr = addr_got + opnd_get_disp(instr_get_src(&inst, 0));
            dr_fprintf(f_global, "\tfound return value "PFX" for indir func %s @"PFX"\n",
                addr, replace_routine_name[index], prev_pc);
            if (addr < mod->start || addr > mod->end) {
                dr_fprintf(f_global, "WARNING: unknown code in indir func %s @"PFX"\n",
                    replace_routine_name[index], prev_pc);
                break;
            }
            if (addr != resolved)
	...
	for (i=0; i<REPLACE_NUM; i++) {
        ...
        if (dr_get_proc_address_ex(mod->handle, replace_routine_name[i],
            ...
            if (info.is_indirect_code) {
                ...
                dr_fprintf(f_global, "export %s indirected from "PFX" to "PFX"\n",
